...
class YandexDisk:
    ...
    def _raise_with_context(
        ...
        if status == 404 and self._looks_like_yandex_share(public_url):
            hint = (
                "404 from Yandex Disk API. Telemost recordings may require OAuth "
                "authentication even for public-looking links. Set YANDEX_DISK_TOKEN "
                "or provide --account/--token-file."
            )
    ...
    def get_public_meta(self, public_url: str) -> dict:
        ...
        resp = self.session.get(endpoint, params={"public_key": public_url})
        try:
            resp.raise_for_status()
        except requests.HTTPError as e:
            self._raise_with_context(e, public_url)
        data = resp.json()
