...
# ── Business logic moved from yandex-mail ─────────────────────────────────────
...
def find_meeting_dir_by_uid(output_base: Path, meeting_uid: str | None) -> Path | None:
    """Find existing meeting directory by meeting UID."""
    if not meeting_uid or meeting_uid == "unknown" or not output_base.exists():
        return None
    for candidate in sorted(output_base.glob(f"*/*_{meeting_uid}")):
        if candidate.is_dir():
            return candidate
    return None


def resolve_meeting_output_dir(meeting_data: dict, output_base: Path) -> Path:
    """Resolve stable output directory: first existing by UID, else canonical path."""
    meeting_uid = meeting_data.get("meeting_uid") or "unknown"
    existing = find_meeting_dir_by_uid(output_base, meeting_uid)
    return existing if existing else build_meeting_output_path(meeting_data, output_base)
...
def _merge_existing_meta(existing: dict, fresh: dict) -> dict:
    ...
    scalar_keys = ("meeting_uid", "title", "reference_utc", "telemost_summary", "video_url", "audio_url")
...
def process_meeting(
    ...
    # Output directory: first existing dir by meeting_uid, else create canonical one.
    meeting_dir = resolve_meeting_output_dir(meeting_data, output_base)
...
def download_recordings(meeting_data: dict, meeting_dir: Path) -> list[dict]:
    ...
    # Resolve account from meeting metadata (first source email's mailbox)
    account = None
    for src in meeting_data.get("source_emails", []):
        if src.get("dir_name"):
            # dir_name format: "YYYY-MM-DD_{mailbox}_uid{N}"
            parts = src["dir_name"].split("_")
            if len(parts) >= 2:
                account = parts[1]
                break
...
def report_result(result: dict, meeting_data: dict, verbose: bool = False) -> str:
...
def main():
    ...
    if not emails:
        print("No enriched Telemost emails in incoming/")
        return
    ...
    for meeting_uid, group_emails in groups.items():
        ...
        for em in sorted_emails:
            ...
            if result:
                ...
                report_result(result, meeting_data, verbose=args.verbose)
