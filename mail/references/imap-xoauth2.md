...
1. Obtain OAuth token via Yandex OAuth (run `python scripts/oauth_setup.py ...` from the Yandex skill root)
...
- No refresh token mechanism — re-run `python scripts/oauth_setup.py ...` from the Yandex skill root to get a new token
