...
- Identity-resolution pitfall: a successful code exchange can import into an existing alias for the verified Yandex identity (for example `ctiis`) rather than the requested hint alias (for example `test`). After completion, inspect the printed resolved alias and account `apps`; report the actual alias authorized, not just the requested alias.
