...
- Alias routing: when `--account <alias>` is supplied to completion or managed import, write the token to that exact alias. If the verified Yandex identity/email already exists under another alias, keep that existing file unchanged, write the requested alias, and emit a warning that names the conflict and the requested write target.
...
During alias-routing tests, `--account <alias>` is an explicit write target, not a hint. If the verified Yandex identity already maps to a different local alias, the requested alias must still receive the new token; the pre-existing alias must remain unchanged. Verify this by checking the JSON report's `saved_account`, `apps`, and `token_path`, then checking the requested alias with `oauth_setup.py --account <alias>`.
