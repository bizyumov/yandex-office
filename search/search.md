---
name: search
description: Search / Поиск — search the web using Yandex Cloud Search API v2. Supports async queries with regional targeting, content filtering, and structured output. Use when Russian-language search results are preferred or when Western search APIs are unavailable.
license: MIT
...
- **Original attribution:** Core API logic based on search by Fedor Kondakov (MIT)
